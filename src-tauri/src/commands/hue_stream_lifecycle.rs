...
//! the data plane (frame, dtls, sender), the runtime owner / DTOs / state
//! machine helpers (state_store, retry, reconnect) now live as siblings under
//! `commands::hue::*`. The Tauri commands themselves (and the public
//! re-exports the rest of the crate already imports) stay in this file
//! during the v1.5 refactor — they will move to `commands::hue::commands`
//! in a follow-up commit, leaving this file as a thin re-export shim.
...
use super::hue::state_store::{
    channels_to_info_via_owner, flush_pending_solid_color, make_result,
    status_with,
};
use super::hue_onboarding::{check_hue_stream_readiness, CommandStatus};

// ---------------------------------------------------------------------------
// Re-exports — keep external `super::hue_stream_lifecycle::*` import paths
// stable for `lib.rs`, `lighting_mode.rs`, and `runtime_telemetry.rs`.
// ---------------------------------------------------------------------------
...
#[allow(unused_imports)]
pub(crate) use super::hue::state_store::{acquire_hue_runtime, HueActiveStreamContext, HueRuntimeOwner};
...
pub async fn get_hue_area_channels(
    ...
    Ok(super::hue::frame::channels_to_info(&channels))
}
...
mod tests {
    ...
    use super::super::hue::dtls::hex_decode;
    use super::super::hue::frame::{build_huestream_frame, HueColorUpdate};
    ...
    // Local Mutex re-export so tests can keep using `std::sync::Mutex` without
    // a separate import in every test fn.
    #[allow(dead_code)]
    fn _silence_mutex<T>(_: &Mutex<T>) {}
    ...
    fn transient_faults_exhaust_into_failed_and_auth_invalid_sets_repair_hint() {
        use super::super::hue::retry::register_auth_invalid;
