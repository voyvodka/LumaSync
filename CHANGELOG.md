...
- The Hue panel on the Devices page is now a short list of coarse placements — left, right, near, far and centre — rather than a drag pad with a height slider. Free positioning belongs to the room map, which draws your actual room; this was a second, smaller surface for the same job, and its height slider read -1 to 1 with no unit. Each button now writes a position, so it reaches the lights as soon as you press it — the buttons used to record a label the lighting never read. Any label you had set is converted into the position it always meant, so a light that carried one, and never followed it, moves on the first launch after this update. Each row is labelled with the number the bridge itself gives that light, shows Custom when you have placed it somewhere no button covers, and outlines rather than fills the button when the placement came from the bridge instead of from you.
