...
- Devices → USB Strips is one path from top to bottom: the controller (your supported ports, each
  ...
  that reconnected on its own but is not in the list yet gets an "Add connected strip" button. A
  second controller gets a strip of its own instead of taking over the one you drew for the first,
  and the room map still opens on its templates after a connect, keeping the connected strip when
...
- Lighting: a mode restored at launch now reaches a USB strip or WLED device that finishes
  ...
  selected, and says the strip is not connected, instead of doing nothing; a Hue-only choice that
  Hue refuses now says why (bridge unreachable, pairing needed, another app using the area, no
  lights in the area). A Hue stream that another app takes over is reported as that instead of
  "retry budget exhausted". Picking another entertainment area while streaming moves the stream to
  it. A saved LED layout reaches the running mode, and colour correction, colour order and display
  changes reach the strip while Hue reconnects. Off and the output switches are never locked
  behind the LED layout, a Hue-only setup is no longer asked for a layout for a strip it does not
  have, finishing Hue setup turns the Hue output on, the "continuing on the other outputs" notice
  no longer appears while lighting is off, and the tray's labels and the rest of the window no
  longer wait for the launch's lighting restore.
...
- Devices → USB Strips: serial ports that cannot drive LEDs, such as the Bluetooth port every Mac
  ...
  of showing a generic error with English text, and a Rescan pressed too soon says to wait until
  the next rescan completes. Port cards no longer show a made-up baud rate, protocol and latency.
  The SK6812 + Adalight warning on the chip type now appears, the chip type tiles show a keyboard
  focus ring, and the spacing and type on the page follow one rhythm. With the keyboard, the
  firmware profile can always be reached and says on the tile why a profile is unavailable,
  "Use anyway" switches from its words too, closing Change port returns to its button, and screen
  readers hear which strip each row's buttons belong to, each connection status once, and each
  setting's name once. Adding a strip no longer risks undoing a Hue channel change saved at the
  same moment.
- Lights: the USB and Hue output toggles no longer look switched on beside "No strip connected"
  or "Not configured". The saved choice is kept and shows again once the output is back, and the
  last output that can actually light can no longer be switched off.
...
- Saving Hue light positions to the bridge now works. It never did: the bridge turned away every
  ...
  bridge stores them. A light whose position the bridge works out for itself, such as one section
...
- A Hue zone dragged against a wall now shows which wall stopped it. A zone cannot leave the room, so its handle stops as soon as the box edge reaches a wall — but the handle sits at the centre, which on a large zone is metres away from that edge, so the drag looked stuck for no visible reason. The edge that is holding it now lights up while you drag.
- The room map opens framed on your actual room again. It was always sized as though the room were the 5 × 4 m default, because the view was fitted before the saved room had finished loading — so any larger room opened too far zoomed in, running off the edge of the canvas instead of sitting centred in it. Zooming or panning afterwards still behaves as before, and resizing the room still leaves your view where you put it.
- Two Hue entertainment areas no longer overwrite each other's light positions. Positions were stored under the light's number alone, and that number restarts at zero in every area — so the second area's first light and the first area's first light were the same record, and whichever you arranged last replaced the other. Each position now records the area it belongs to; existing arrangements are assigned to the area you last worked in, which is the only one that could have written them. The room map shows one area at a time, since selecting a light there still identifies it by number alone.
- Locking a Hue light no longer comes undone when you put it in a zone. The lock was not part of what the zone editor sent back, so it was dropped on the way through.
...
- Resizing the window by dragging its edge is now remembered. Only the size the window had when you last switched between the compact and wide layouts was ever saved, so any resize you made afterwards was lost on the next launch and the window went back to that older size. The compact layout keeps its fixed size, as before.
...
- The log file no longer records every line twice, and no longer leaves a second, differently-cased copy of itself beside the first. The logger was given its two outputs on top of the two it already had by default, so each line went to the terminal twice and, in a release build, to the same file through two handles — which also made the file look twice as busy as the app was.
- On Windows, the preview and calibration overlays no longer open blank and leave the app unreachable. To let clicks through, the overlay marked every window the browser engine had created inside it — but one of the two flags it used tells Windows the window must not be drawn until the program supplies it with a transparency setting, which is not something we can do for a window the browser engine owns. So the overlay covered the screen showing nothing, and since it carries no title bar and stays out of the taskbar there was little left to close it with. That flag is gone; letting clicks through never depended on it, and the overlay window itself already passes them to whatever is behind it. Two smaller faults on the same path went with it: the overlay asked for a fully transparent background, which Windows reads as solid black and paints over the whole screen, and the overlay was created while the app held a lock the browser engine could re-enter during creation — enough on its own to freeze the app before anything appeared. The overlay also no longer takes keyboard focus when it opens. Reproduced and confirmed on a Windows machine in the build pipeline — the old marking turns the whole display solid black, the new one draws the overlay over the desktop and lets clicks through — and that check now runs on every change.
...
- Where you place a Hue light in the room map is now what the app samples from. Until now the lights followed the positions stored on the bridge, so arranging them here changed nothing until you pushed those positions to the bridge and the app read them back — and the coarse buttons on the Devices page never reached the lights at all, on any path. Your placement is sent when the stream starts, so it takes effect the next time lighting comes on. Writing positions to the bridge stays a separate, deliberate action, since that layout is shared with the Hue app and anything else that streams — though in this release the bridge still refused that write.
- The room map calls each Hue light by the number the bridge gives it, and says how many bulbs it drives. It was numbering them by their place in the list and adding one — so on any area with a gap in its numbering, every name on the map was wrong, and it disagreed with the Devices page besides. A light the app has not matched to the bridge now says so instead of borrowing a number, and one the bridge has stopped reporting explains why it is still on the map.
- The Hue panel in the room map no longer squeezes its own status line into one word per line. The re-read button was placed beside the text without allowing the row to wrap, so in the 280-pixel dock the heading broke after every word and the line under it was cut off mid-word — worse in Turkish, where the label is longer. The button now drops to its own line when it does not fit. In the same panel, the zone's edge and colour fields no longer split a label from the box it names when the row wraps, and on the Devices page a channel's light count is shortened rather than pushed out of its column.
- The Devices page now says whether the bridge holds the same light arrangement you do, and lets you settle it either way. Until now the only clue was your own memory of whether you had pressed save, and closing the app lost even that. It is deliberately not phrased as a fault when the two differ: your lights already follow your arrangement, and what is behind is the copy the Hue app and a Sync Box read. Alongside the existing send (which the bridge still refused in this release) there is now a take-the-bridge's, which asks first because it replaces what you have. Both are held back while lighting is running, since the arrangement in play at that moment is yours rather than the bridge's.
- The room map reads your Hue bridge itself, instead of waiting for the Devices page to tell it what exists. Until now the lights only appeared on the map after that page had been opened at least once — so a fresh install could open the room map to an empty room while the bridge was streaming perfectly. A light added to the entertainment area now shows up on the map, a light removed from it is drawn as a dashed outline rather than silently lingering as though it were still there, and a button re-reads the list without restarting the app. A bridge that stops answering leaves the lights exactly as they were.
- An entertainment area you emptied in the Hue app now reads as empty on the room map, instead of as a bridge that stopped answering — and the lights left behind on the map are shown as leftovers rather than as though they were still there. A failed read is told apart from both: it leaves the lights exactly as they were and claims nothing about them. The room map's object list also shows the same area the map itself does; it was listing every area's lights, and because a light is identified by its number alone, picking one there could select a different area's.
- The Devices page no longer offers a second place to position Hue lights. It listed five coarse buttons — left, right, near, far, centre — beside the room map's free placement, which is two surfaces for one job and the smaller of the two could never express what the room map can: distance and height in metres. The page now lists what the bridge reports for the area — each light's number, how many bulbs it drives, and which zone it belongs to — keeps the bridge sync, and links to the room map. Any position you set with those buttons is kept; only the buttons are gone.
...
- Resolved RUSTSEC-2026-0285 (5.3 medium) by bumping the transitive `rustls` crate 0.23.44 → 0.23.45; TLS 1.3 handshake messages were accepted across encryption level boundaries. The crate reaches LumaSync through the `reqwest` chain used for Hue CLIP v2 and the updater. The `cargo audit` CI gate is green again.
