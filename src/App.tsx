...
function App() {
  ...
  // Assigned during render below, so a tray event arriving before the effect
  // flush still reaches the current handler.
  const handleLightingModeChangeRef = useRef<((m: LightingModeConfig) => Promise<void>) | null>(null);
  useTrayIntegration({
    lightingModeChangeRef: handleLightingModeChangeRef,
    lightingModeRef,
    lastNonOffModeRef,
