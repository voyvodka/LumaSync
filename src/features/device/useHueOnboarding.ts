...
export function useHueOnboarding(): UseHueOnboardingResult {
  ...
  useEffect(() => {
    ...
    // Visibility-aware recursive setTimeout prevents background polling
    // when the window is hidden, saving CPU and network resources.
    const scheduleNext = () => {
      if (!active || document.visibilityState === "hidden" || timeoutId !== null) return;
