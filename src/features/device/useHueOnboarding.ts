...
export function useHueOnboarding(): UseHueOnboardingResult {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Replaced setInterval with a visibility-aware recursive setTimeout
    // to prevent redundant bridge polls when the app is hidden and ensure overlapping
    // requests do not occur.
    let active = true;
