...
export function useHueOnboarding(): UseHueOnboardingResult {
  ...
  useEffect(() => {
    ...
    // Visibility-aware recursive setTimeout for background readiness polling
    void tick();
  ...
  }, [applyReadinessResult, selectedBridge, state.credentials, state.isValidatingCredential, state.isLoadingAreas, state.selectedAreaId]);
