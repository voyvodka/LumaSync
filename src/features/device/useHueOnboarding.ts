...
export function useHueOnboarding(): UseHueOnboardingResult {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Use recursive setTimeout instead of setInterval.
    // This allows us to pause readiness polling when the application window
    // is hidden, reducing unnecessary network overhead and saving resources.
    const scheduleNext = () => {
      if (!active) return;
