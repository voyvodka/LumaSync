...
export function useHueOnboarding(): UseHueOnboardingResult {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (!active) return;
      // ⚡ Bolt: Prevent overlapping network requests if the response is slow
      if (inFlight) return;
      // ⚡ Bolt: Pause polling when the application is hidden to save CPU and network bandwidth
    ...
    const handleVisibilityChange = () => {
      ...
      if (document.visibilityState === "visible" && timeoutId === null && !inFlight) {
        // ⚡ Bolt: Instantly resume polling when the app becomes visible
        void run();
      }
    };
