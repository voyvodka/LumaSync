...
export function useHueOnboarding(): UseHueOnboardingResult {
  ...
  useEffect(() => {
    ...
    // Replaced simple setInterval with a visibility-aware recursive setTimeout
    // to pause background readiness checks when the app is hidden/minimized,
    // reducing network and CPU overhead.
    const tick = async () => {
      if (!mounted) return;
      if (inFlight) return;
