...
export function useHueOnboarding(): UseHueOnboardingResult {
  ...
  useEffect(() => {
    ...
    // Bolt Performance Optimization: Replace unconditional setInterval with visibility-aware
    // recursive setTimeout to prevent unnecessary network requests when the app is hidden.
    const scheduleNext = () => {
      if (!active) return;
