...
(`gh issue list`, `gh pr list`). The maintainer also keeps a private roadmap under `.planning/`,
which is gitignored and present only on their machine — never assume it is available, never
reference its paths in commits, PRs, or release notes, and never propose committing it.
