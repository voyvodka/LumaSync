...
- **On macOS only the twin overlay and the control popup show over another app's fullscreen Space.** `elevate_overlay_window` (`macos_window.rs`, called from `led_preview.rs`) gives them `CanJoinAllSpaces | FullScreenAuxiliary` and `NSStatusWindowLevel`, because the preview is used against a fullscreen game or video. The calibration overlay (`calibration.rs`) is only `always_on_top`, which does not reach a fullscreen Space. A new overlay that has to show over fullscreen content needs the same call.
