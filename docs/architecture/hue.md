...
**The types that carry keys write their own `Debug`.** `StartHueStreamRequest`,
`HueActiveStreamContext`, `HueLightRestore` and the resolved credential types redact the app key and
client key by hand. The log file is what users attach to issues, and a derived `Debug` puts the keys
into any `{:?}` line. The cost is that a new field has to be added to the impl by hand; trusting
review to catch a debug print was the rejected option.
...
- **What the 16-bit RGB on the wire means is not documented.** Signify's Entertainment API page
  ...
  (grey ramp beside the screen), not something this code can settle. A ramp judged by eye on
2026-09-25 read 128 as mid-grey beside 255. That does not settle it, and it does not point towards
encoded: stage 6 has already put 128 on the wire as about 0.22, which a bridge taking the value as
light shows as mid-grey, while a bridge decoding it again would show it clearly dim. What would
settle it is numeric — stream a few known solid colours and read the bulb's `dimming` and
`color.xy` back over CLIP v2, against what each reading predicts. If the bridge turns out to
...
**Zones are Hue-only.** The v1.5 W4-F2 reversal collapsed a generic `Zone` discriminated by
