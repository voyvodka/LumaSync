...
- **The worker follows the live stream, not the one it started with.** `HueRuntimeOwner` owns a
  ...
  - **Still on the frontend.** The orderings above stay as belt-and-braces and go in a later
    change. Off still has to call `stop_lighting`: Rust does not end a mode because its Hue output
    ended, so a Hue-only worker would go on capturing the screen with nothing to send. The test
    lease still leaves a stream a running mode adopted to that mode, since stopping it would turn
    off the mode's Hue output, even though the stop itself would now complete.
