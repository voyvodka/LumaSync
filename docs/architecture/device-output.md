...
- **`useDeviceConnection` is mounted three times, each with its own controller instance:** `App.tsx` (Lights surface, StatusBar pill, `usbConnected` prop), `DeviceSection.tsx` (the pair UI), and `control/FirmwareProfilePicker.tsx` (reads `latestHealthCheck.advertisedFirmwareProfile` to know which profile the connected firmware advertised). Each mount runs its own boot-time port scan and, when a `lastSuccessfulPort` is persisted, its own auto-reconnect attempt — so a cold launch does that work three times concurrently instead of once. `connectionEvents.ts`'s pub-sub bridges *state* (a pair done in one mount is mirrored to the others), it does not dedupe the *scan or reconnect* itself. Collapsing the three mounts to a singleton would fix this, but it is a real architecture change: six controller test fixtures build a fresh controller per scenario and assume that shape. Tracked as part of the device-hook decomposition package rather than fixed inline. The third mount exists only to read one derived field (`latestHealthCheck`); if that field moved to a lighter shared source (e.g. the connection-events bus carrying the last health-check result, or lifting `latestHealthCheck` into shell state), `FirmwareProfilePicker` could drop the full hook and the redundant scan/reconnect would go with it.
