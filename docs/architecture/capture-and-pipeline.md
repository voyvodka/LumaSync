...
**A saved display that no longer resolves falls back to the primary, and says nothing.**
`list_displays` builds each id as `<name>:<x>:<y>` from Tauri's monitor API, which no capture API
...
surface it rather than remove it.
...
- **The Windows frame includes the cursor; the macOS frame does not.** ScreenCaptureKit is configured with `with_shows_cursor(false)`. WGC gets `CursorCaptureSettings::Default`, which leaves the session's cursor capture at the system default, and that default is on, so a pointer parked at a screen edge can land in an edge LED's sample. `WithoutCursor` is not a drop-in fix: where `GraphicsCaptureSession.IsCursorCaptureEnabled` is absent (older Windows 10 builds), `windows-capture` refuses to start the session with `CursorConfigUnsupported`, so the setting has to be gated on `is_cursor_settings_supported()`.
...
- **The UI learns about a stall by push, not by polling telemetry.** `get_runtime_telemetry` is only polled while Settings → General "Show stats for nerds" is on, and it is off by default, so the stall notice (and the Lights page's serial link-budget note) cannot ride that poll. Each flush instead derives a `RuntimeHealth` — the failure reason while it is younger than `CAPTURE_FAILURE_ONGOING_MAX_AGE_SECS`, plus the link budget — and `RuntimeTelemetryWindow` emits `telemetry://health-changed` to the main window only when it differs from what that worker last sent: once at the worker's first flush (which also overwrites whatever the previous worker left), once when a stall starts, once when it clears. A worker that ends having reported something sends the empty value from its `Drop`. The emitter is a process-wide sink registered in `lib.rs` setup, because the worker is handed only the snapshot `Arc`; a worker started before registration publishes nothing. The frontend attaches one listener per window for the app's lifetime and seeds it with a single `get_runtime_telemetry` read, which covers a WebView reload in the middle of a stall — the worker will not repeat itself until something changes. Cost: nothing per frame; one comparison per one-second flush, and an event only on a change.
- **Collection stays on with the setting off, because it costs nothing worth gating.** Per frame the worker adds two or three `u32` increments, one `f32` store into a struct it owns and one `Instant::now()` for the flush check — no lock, no allocation; once a second it locks an uncontended mutex to publish, cloning a string only while capture is failing. A standalone `rustc -O` replica of that bookkeeping measured about 20 ns per frame on an Apple-silicon Mac, 15 ns of it the clock read: roughly 1 µs per second at 60 fps, against a 50 ms frame interval. Hue adds one relaxed atomic increment per packet. None of it is worth a flag, and the stall push above depends on the same window. What the setting removes is the reading side: the 1 Hz `get_runtime_telemetry` IPC, its JSON round trip, and the Hue runtime lock plus `format!` it takes per call.
