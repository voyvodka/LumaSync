...
**A channel's height carries its provenance, because a stored `z = 0` does not say whether anyone measured it.** Until heights were carried end to end, seeding wrote `z: 0` for every new channel whatever the bridge reported, and "Save to bridge" pushed that `0` back — so saving positions reset every light's height on the bridge. `zOrigin` (`"bridge"` | `"user"`, absent ⇒ unknown) settles it without a migration or a schema bump, since an absent field already means "unknown". A new record takes the bridge's `z` stamped `"bridge"`, or `0` with no origin when the bridge reports none. A legacy record with no origin and `z === 0`, not bound to a zone, is a seeding placeholder: it adopts the bridge's height when there is one. A legacy non-zero `z` was authored locally — the height slider, or a zone the channel has since left — so it is stamped `"user"`, and every later slider write (`setHueChannelWorldZ`) stamps `"user"` too. A zone-bound legacy record is left alone — its height is the zone's projection, not a seeded default, and adopting the bridge's would quietly detach it. Two paths honour an unknown origin rather than trusting the number: the start request (`toChannelPlacements`) omits `positionZ` so the stream keeps the bridge's own height, and the write-back reads the bridge's current `z` and sends that instead, refusing the push if the read fails (the bridge's position object takes `x`, `y` and `z` together, so the height cannot just be left out). The persisted `z` stays a plain `number`; nothing samples it yet — region, topology and screen affinity still read only `x`/`y` — so carrying it changes no colour.
